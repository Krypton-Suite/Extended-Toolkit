...
using Krypton.Toolkit.Extended.Palette.Controls.Settings;
...
namespace Krypton.Toolkit.Extended.Palette.Controls
{
    public class AllMergedPaletteColourSettingsManager
    {
        ...
        {
            if (KryptonMessageBoxExtended.Show("WARNING! You are about to reset these settings back to their original state. This action cannot be undone!\nDo you want to proceed?", "Reset Settings", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
            {
        ...
        public void SaveAllMergedColourSettings(bool alwaysUsePrompt = false)
        {
            if (alwaysUsePrompt)
            {
                if (KryptonMessageBoxExtended.Show("You have changed a setting value. Do you want to save these changes?", "Setting Values Changed", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
