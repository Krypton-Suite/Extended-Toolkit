    ...
    public class FileTextSource : TextSource, IDisposable
    {
        ...
        public void OpenStream(Stream stream, Encoding enc)
        {
            ...
            if (!stream.CanRead || !stream.CanSeek)
                throw new ArgumentException("Stream must allow read and seek");

            // close possibly opened stream unless its the passed in stream 
            if (fs != null && !ReferenceEquals(fs, stream))
                fs.Dispose();
            fs = stream;
            SaveEOL = Environment.NewLine;
