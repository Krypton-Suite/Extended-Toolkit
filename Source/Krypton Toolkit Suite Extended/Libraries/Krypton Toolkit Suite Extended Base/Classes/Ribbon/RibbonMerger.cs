...
namespace Krypton.Toolkit.Extended.Base
{
    ...
    {
        #region Variables
        // List of the merged items
        HashSet<Component> _mergedItems = new HashSet<Component>();
        #endregion
        ...
        private int GetSortIndexFromTag(object tagValue, int maxValue)
        {
            ...
            if (tagValue == null) return maxValue;

            if (tagValue is string)
            {
                if (int.TryParse(tagValue.ToString(), NumberStyles.Integer, CultureInfo.CurrentCulture, out int parsedValueResult))
                {
        ...
        private void UnmergeGroupItems(KryptonRibbonGroupContainerCollection sourceItems, KryptonRibbonGroupContainerCollection targetItems)
        {
            ...
            {
                if (!_mergedItems.Contains(item)) continue;
        ...
        private void UnmergeGroups(KryptonRibbonGroupCollection sourceGroups, KryptonRibbonGroupCollection targetGroups)
        {
            ...
            {
                if (!_mergedItems.Contains(group))
                {
                    KryptonRibbonGroup existingGroup = sourceGroups.FirstOrDefault(item => (string.Equals(item.TextLine1, group.TextLine1, StringComparison.CurrentCulture)) && (string.Equals(item.TextLine2, group.TextLine2, StringComparison.CurrentCulture)));

                    if (existingGroup != null) UnmergeGroupItems(existingGroup.Items, group.Items);

                    continue;
                }

                _mergedItems.Remove(group);
