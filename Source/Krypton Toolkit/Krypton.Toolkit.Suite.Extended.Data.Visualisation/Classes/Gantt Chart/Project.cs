namespace Krypton.Toolkit.Suite.Extended.Data.Visualisation
{
        ...
        where R : class
    {
        ...
        public void Relate(T precedent, T dependant)
        {
                ...
                )
            {
                    ...
                    )
                {
                    ...
                    RecalculateDependantsOf(precedent);
                    RecalculateAncestorsSchedule();
                    RecalculateSlack();
                }
            }
        ...
        public void SetStart(T task, TimeSpan value)
        {
            ...
            {
                SetGroupStartHelper(task, value);
        ...
        public void SetComplete(T task, float complete)
        {
            ...
            {
                SetCompleteHelper(task, complete);
        ...
        /// </summary>
        /// <param name="task"></param>
        private void DetachTask(T task)
        {
        ...
        private void SetStartHelper(T task, TimeSpan value)
        {
            if (task.Start != value)
            {
        ...
        private void SetGroupStartHelper(T group, TimeSpan value)
        {
            if (_mRegister.Contains(group) && value != group.Start && this.IsGroup(group))
            {
                ...
                foreach (T decendant in decendants)
                {
                    ...
                    RecalculateDependantsOf(decendant);
                }
        ...
        private void SetEndHelper(T task, TimeSpan value)
        {
            if (task.End != value)
            {
                if (_mSplitTaskOfPart.ContainsKey(task))
        ...
        private void SetPartStartHelper(T part, TimeSpan value)
        {
            ...
            // pack packs
            if (backwards) PackPartsBackwards(parts);
            else PackPartsForward(parts);
        ...
        private void SetPartEndHelper(T part, TimeSpan value)
        {
            var split = _mSplitTaskOfPart[part];
            var parts = _mPartsOfSplitTask[split];
            ...
            // pack parts
            if (increased) PackPartsForward(parts);
        ...
        private void PackPartsBackwards(List<T> parts)
        {
        ...
        private void PackPartsForward(List<T> parts)
        {
        ...
        private void SetCompleteHelper(T task, float value)
        {
        ...
        private void RecalculateComplete()
        {
            Stack<T> groups = new Stack<T>();
            foreach (var task in _mRootTasks.Where(x => this.IsGroup(x)))
            {
                RecalculateCompletedHelper(task);
            }
        }

        private float RecalculateCompletedHelper(T groupOrSplit)
        {
            float t_complete = 0;
            ...
            else
            {
                foreach (var member in this.DirectMembersOf(groupOrSplit))
                {
                    ...
                    if (this.IsGroup(member)) t_complete += RecalculateCompletedHelper(member) * member.Duration.Ticks;
        ...
        private void RecalculateDependantsOf(T precedent)
        {
            // affect decendants
            foreach (var dependant in this.DirectDependantsOf(precedent))
            {
                if (dependant.Start < precedent.End)
                    this.SetStartHelper(dependant, precedent.End);
            }
        }

        private void RecalculateAncestorsSchedule()
        {
            // affects parent group
            foreach (var group in _mRootTasks.Where(x => this.IsGroup(x)))
            {
                RecalculateAncestorsScheduleHelper(group);
            }
        }

        private void RecalculateAncestorsScheduleHelper(T group)
        {
            float t_complete = 0;
            TimeSpan t_duration = TimeSpan.Zero;
            ...
            foreach (var member in this.DirectMembersOf(group))
            {
                if (this.IsGroup(member))
                    RecalculateAncestorsScheduleHelper(member);
            ...
            this.SetStartHelper(group, start);
            this.SetEndHelper(group, end);
            this.SetCompleteHelper(group, t_complete / t_duration.Ticks);
        }

        private void RecalculateSlack()
        {
