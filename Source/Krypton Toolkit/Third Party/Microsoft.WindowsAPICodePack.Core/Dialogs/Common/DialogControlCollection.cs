namespace Microsoft.WindowsAPICodePack.Core
{
    ...
    public sealed class DialogControlCollection<T> : Collection<T> where T : DialogControl
    {
        ...
        protected override void InsertItem(int index, T control)
        {
            ...
            {
                throw new InvalidOperationException(LocalisedMessages.DialogCollectionCannotHaveDuplicateNames);
            }
            if (control.HostingDialog != null)
            {
                throw new InvalidOperationException(LocalisedMessages.DialogCollectionControlAlreadyHosted);
            }
            if (!hostingDialog.IsCollectionChangeAllowed())
            {
                throw new InvalidOperationException(LocalisedMessages.DialogCollectionModifyShowingDialog);
            }
        ...
        public T this[string name]
        {
            get
            {
                ...
                {
                    throw new ArgumentException(LocalisedMessages.DialogCollectionControlNameNull, "name");
                }
