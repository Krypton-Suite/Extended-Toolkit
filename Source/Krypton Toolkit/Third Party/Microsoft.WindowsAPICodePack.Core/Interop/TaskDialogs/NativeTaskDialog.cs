namespace Microsoft.WindowsAPICodePack.Core
{
    ...
    internal class NativeTaskDialog : IDisposable
    {
        ...
        internal void NativeShow()
        {
            ...
            if (settings == null)
            {
                throw new InvalidOperationException(LocalisedMessages.NativeTaskDialogConfigurationError);
            }
            ...
            try
            {
                ...
                if (InternalCoreErrorHelper.Failed(hresult))
                {
                    string msg;
                    switch (hresult)
                    {
                        case HResult.InvalidArguments:
                            msg = LocalisedMessages.NativeTaskDialogInternalErrorArgs;
                            break;
                        case HResult.OutOfMemory:
                            msg = LocalisedMessages.NativeTaskDialogInternalErrorComplex;
                            break;
                        default:
                            msg = string.Format(System.Globalization.CultureInfo.InvariantCulture,
                                LocalisedMessages.NativeTaskDialogInternalErrorUnexpected,
                                hresult);
                            break;
                    }
            ...
            {
                throw new NotSupportedException(LocalisedMessages.NativeTaskDialogVersionError, exc);
            }
            finally
            {
        ...
        private int PerformDialogInitialization()
        {
            ...
            return InternalCoreErrorHelper.Ignored;
        }
