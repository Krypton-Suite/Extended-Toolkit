    ...
    public class HalfHourCell : DataGridViewTextBoxCell
    {
        ...
        protected override void Paint(Graphics graphics, Rectangle clipBounds, Rectangle cellBounds, int rowIndex, DataGridViewElementStates cellState, object value, object formattedValue, string errorText, DataGridViewCellStyle cellStyle, DataGridViewAdvancedBorderStyle advancedBorderStyle, DataGridViewPaintParts paintParts)
        {
            try
            {
                ...
                if (IsWorkingTime)
                    graphics.FillRectangle(new SolidBrush(CalendarControl.ct.WorkingColour), cellBounds);
                else
                    graphics.FillRectangle(new SolidBrush(CalendarControl.ct.NonWorkingColour), cellBounds);

                if (this.Selected)
                    graphics.FillRectangle(new SolidBrush(CalendarControl.ct.SelectedBackColour), cellBounds);

                int leftMargin = 4;
                int rightMargin = 3;

                // Draw the left margin vertical line and fill
                Pen pen = new Pen(new SolidBrush(CalendarControl.ct.BorderColour));
                graphics.DrawLine(pen, cellBounds.Left + leftMargin, cellBounds.Top, cellBounds.Left + leftMargin, cellBounds.Bottom);
                graphics.FillRectangle(new SolidBrush(CalendarControl.ct.LeftMarginFillColour), cellBounds.Left, cellBounds.Top, leftMargin, cellBounds.Height);

                // Draw the bottom end of cell
                if (this.EndTime.Minute == 0)
                    graphics.DrawLine(new Pen(new SolidBrush(CalendarControl.ct.HourEndingColour)), cellBounds.Left + leftMargin + 1, cellBounds.Bottom - 1, cellBounds.Right, cellBounds.Bottom - 1);
                else
                    graphics.DrawLine(new Pen(new SolidBrush(CalendarControl.ct.HalfHourEndingColour)), cellBounds.Left + leftMargin + 1, cellBounds.Bottom - 1, cellBounds.Right, cellBounds.Bottom - 1);
                ...
                for (int i = 0; i < items.Count; i++)
                {
                    ...
                    if (this.EndTime.Minute == 0)
                        graphics.DrawLine(new Pen(new SolidBrush(CalendarControl.ct.HourEndingColour)), startX + leftMargin + 1, cellBounds.Bottom - 1, startX + leftMargin + widthForItem + rightMargin, cellBounds.Bottom - 1);
                    else
                        graphics.DrawLine(new Pen(new SolidBrush(CalendarControl.ct.HalfHourEndingColour)), startX + leftMargin + 1, cellBounds.Bottom - 1, startX + leftMargin + widthForItem + rightMargin, cellBounds.Bottom - 1);
                    ...
                    LinearGradientBrush lgb = new LinearGradientBrush(ItemRect, ColourTable.GetLightColour(items[i].Colour, 110), items[i].Colour, LinearGradientMode.Horizontal);
                    ...
                    {
                        HatchBrush hb = new HatchBrush(HatchStyle.WideUpwardDiagonal, CalendarControl.ct.LeftMarginFillColour, CalendarControl.ct.ItemLeftMarginColour);
                        graphics.FillRectangle(hb, startX, cellBounds.Top, leftMargin, cellBounds.Height);
                    }
                    else
                    {
                        graphics.FillRectangle(new SolidBrush(CalendarControl.ct.ItemLeftMarginColour), startX, cellBounds.Top, leftMargin, cellBounds.Height);
                    }
                    ...
                    if (items[i].StartTime.Equals(this.StartTime))
                    {
                        ...
                        graphics.DrawString(items[i].Description, new Font("Arial", 8), CalendarControl.ct.ForeBrush, p);
                    }
